...
TCHAR*			LogFile::GetWorkFolder()
{
	if(m_szPath[0] == 0)
	{
		...
		TCHAR szFileName[126]={0};
		_sntprintf(szFileName,sizeof(szFileName),TEXT("%slog\\logtext.ini"),m_szPath);

		m_bCanLogText = GetPrivateProfileInt(TEXT("ServerStatus"),TEXT("log"),0,szFileName);
	}
	return m_szPath;
}
