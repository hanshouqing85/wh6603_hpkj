...
//		szText = pDc.GetTextExtent( "TEST" );
		...
		if ( ((_QMTAB_TAB *)m_arTabs.GetAt( i ))->nPageIdx > 0 && GetTabItemCount(i)>nGroupItemCount ) 
		{
			if ( m_iSelectedItem == QMTAB_CFG_ARROWUP ) {
				if ( m_bMouseDown ) {
					m_arrowList.Draw( &pDc, 1, CPoint( rc2.Width()-10+1, 10+1 ), ILD_NORMAL );
				}
				else {
					m_arrowList.Draw( &pDc, 1, CPoint( rc2.Width()-10, 10 ), ILD_NORMAL );
				}
			}
			else {
				m_arrowList.Draw( &pDc, 3, CPoint( rc2.Width()-10, 10 ), ILD_NORMAL );
			}
		}
			...
			&&GetTabItemCount(i)>nGroupItemCount )
		{
			if ( m_iSelectedItem == QMTAB_CFG_ARROWDOWN ) {
				if ( m_bMouseDown ) {
					m_arrowList.Draw( &pDc, 0, CPoint( rc2.Width()-10+1, rc2.Height()-QMTAB_CFG_ARROWHEIGHT-10+1 ), ILD_TRANSPARENT );
				}
				else {
					m_arrowList.Draw( &pDc, 0, CPoint( rc2.Width()-10, rc2.Height()-QMTAB_CFG_ARROWHEIGHT-10 ), ILD_TRANSPARENT );
				}
			}
			else {
				m_arrowList.Draw( &pDc, 2, CPoint( rc2.Width()-10, rc2.Height()-QMTAB_CFG_ARROWHEIGHT-10 ), ILD_TRANSPARENT );
			}
		}
...
int CQMTab::GetMaxTabItemCount()
{
	...
	//change 20 to 10
	int nGroupItemCount = (rc2.Height()-10)/(m_iconSize.cy+QMTAB_CFG_ITEM_HEIGHT)*3;
