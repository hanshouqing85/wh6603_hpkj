...
{
	m_nStartQihao = 1588649;  //8.2官方开奖 少开100期，初始值减掉100
...
{
	m_nStartQihao = 0;  //8.2官方开奖 少开100期，初始值减掉100
...
CString CSD11X5Rule::GetNextExpect(int nDelta)
{
	...
	CString rQh;
	if (((t.GetHour() == 8 && t.GetMinute()>=35)||t.GetHour()>=9) && (t.GetHour() < 22 || (t.GetHour() == 22 && t.GetMinute()<55))){
		int qishu = 1;
		CTime starttime(t.GetYear(),t.GetMonth(),t.GetDay(),8,25,0);
		CTimeSpan tSpan = t - starttime;
...
// 		else
		...
		if(t.GetHour() > 22 ||((t.GetHour() == 22 && t.GetMinute()> 55)))
			t+=CTimeSpan(1L,0,0,0);
		rQh.Format(_T("%d%02d%02d01"), t.GetYear(), t.GetMonth(), t.GetDay());
	}
...
// 		else
		...
		if(t.GetHour() > 22 ||((t.GetHour() == 22 && t.GetMinute()>55)))
			t+=CTimeSpan(1L,0,0,0);
		CTime ctm(t.GetYear(), t.GetMonth(), t.GetDay(), 8, 25, 0);
		return ctm;
	}
}
//离下次开奖时间还剩下的时间-字符串描述
...
CString CBJPK10Rule::GetNextExpect(int nDelta)
{
	...
	CTime startTime = CTime(2017,6,22,0,0,0);
	int startqihao = 624677;
	int nQiHao =0;
	CTime tm;
	theApp.GetTime(tm);
...
// 		else
	...
	OutputDebugString(L"AAA " + strQiHao);
	return strQiHao;
...
CString CKuaiLe8RUle::GetNextExpect(int nDelta)
{
	...
	CTime	tm_now = CTime::GetCurrentTime();
	xD.GetLunarDate(tm_now.GetYear(), tm_now.GetMonth(), tm_now.GetDay(), iLunarYear, iLunarMon, iLunarDay);
	int nRestDays = (iLunarYear - 2017)*4;
