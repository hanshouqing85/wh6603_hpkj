...
IMPLEMENT_DYNAMIC(CPlatformInbox, CDialog)
//map<int,CString> mapContent;
CPlatformInbox::CPlatformInbox(CWnd* pParent /*=NULL*/)
: CDialog(CPlatformInbox::IDD, pParent)
, m_bmpBk(NULL)
...
void CPlatformInbox::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPlatformInbox, CDialog)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_BTN_OK, &CPlatformInbox::OnBnClickedBtnOk)
	ON_BN_CLICKED(IDC_BTN_PRE_PAGE, &CPlatformInbox::OnBnClickedBtnPrePage)
	ON_BN_CLICKED(IDC_BTN_NEXT_PAGE, &CPlatformInbox::OnBnClickedBtnNextPage)
	ON_WM_SHOWWINDOW()
	ON_NOTIFY(NM_DBLCLK, IDC_LIST_LOG, &CPlatformInbox::OnNMDblclkListLog)
	ON_NOTIFY(NM_RCLICK, IDC_LIST_LOG, &CPlatformInbox::OnNMRclickListLog)
	ON_NOTIFY(NM_CLICK, IDC_LIST_LOG, &CPlatformInbox::OnNMLclickListLog)
	ON_BN_CLICKED(IDC_BTN_CLOSE, &CPlatformInbox::OnBnClickedBtnClose)
	ON_COMMAND(ID_ZNX_CHAKAN, &CPlatformInbox::OnPlatformInboxChk)
	ON_COMMAND(ID_ZNX_SHANCHU, &CPlatformInbox::OnPlatformInboxShc)
END_MESSAGE_MAP()


// CPlatformInbox 消息处理程序

void CPlatformInbox::OnPaint()
{
...
BOOL CPlatformInbox::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}
// CZhanghuDlg 消息处理程序
void CPlatformInbox::OnBnClickedBtnClose()
{
...
void CPlatformInbox::OnSize(UINT nType, int cx, int cy)
{
...
void CPlatformInbox::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
void CPlatformInbox::OnBnClickedBtnOk()
{
	if(m_dwTickCount == 0)
	{
...
void CPlatformInbox::OnBnClickedBtnPrePage()
{
	if(m_page == 1)
	{
...
void CPlatformInbox::OnBnClickedBtnNextPage()
{
	if(m_page == m_maxPage)
	{
...
void CPlatformInbox::OnNMDblclkListLog(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CPlatformInbox::OnNMRclickListLog(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	POSITION pos = m_listLog.GetFirstSelectedItemPosition();
	if (pos == NULL) 
	...
	{
		if(strName == _T("系统消息"))
			pMenu->EnableMenuItem(ID_ZNX_SHANCHU,TRUE);
		pMenu->TrackPopupMenu(TPM_LEFTBUTTON, pt.x, pt.y, this);
	}
	*pResult = 0;
}
void CPlatformInbox::OnNMLclickListLog(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	POSITION pos = m_listLog.GetFirstSelectedItemPosition();
	if (pos == NULL) 
...
void CPlatformInbox::OnPlatformInboxChk()
{
	POSITION pos = m_listLog.GetFirstSelectedItemPosition();
	if (pos != NULL) 
	{
		...
		m_nUserID = mapSendUser[t_id];
		SendToServer(2);
		CZhanNeiXinXX dlg;

		CString strUser= m_listLog.GetItemText(nItem, 1);
		
// 		if(strUser == _T("系统消息"))
// 			dlg.SetContent(sTitle,sName,sContent,sTime,2);
// 		else
		...
		if(dlg.DoModal()==IDOK)
		{
			m_strName = dlg.strName;
			m_strTitle = sTitle;
			CWnd* pParent = GetParent();
			if(pParent!=NULL)
			{
				pParent->PostMessage(IDM_REPLY,1,0);
			}
		}
	}
}

void CPlatformInbox::OnPlatformInboxShc()
{
	POSITION pos = m_listLog.GetFirstSelectedItemPosition();
	if (pos != NULL) 
	{
...
BOOL CPlatformInbox::OnInitDialog()
{
...
void CPlatformInbox::OnCancel()
{
	//CDialog::OnCancel();
}

void CPlatformInbox::OnOK()
{
	//CDialog::OnOK();
}

void CPlatformInbox::InitListCtrl()
{
...
void CPlatformInbox::AdjustCtrls()
{
...
//读取事件
bool CPlatformInbox::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
		...
		case SUB_GP_GET_INBOX_MESSAGE_RET:
			{
				...
				for(int i = 0;i < nCount;i++)
				{
					...
					mapSendUser[pLog->n_t_id] = pLog->n_send_userid;
 					if(pLog->n_t_ifSj == 1)
 						m_listLog.SetItemText(item, 1, L"上级");
					else if(pLog->n_t_ifSj == 2)
						m_listLog.SetItemText(item, 1, L"客服");
					else if(pLog->n_t_ifSj == 3)
						m_listLog.SetItemText(item, 1, L"系统消息");
 					else
					...
					if(m_nTypeID ==1)
					{
						...
						if(pLog->c_t_ifread==0)
							m_listLog.SetItemTextColor(3,item,RGB(231 ,76, 69));
						else
							m_listLog.SetItemTextColor(3,item,RGB(45,97,124));
					}
					else
					...
					CString strLog;
					strLog.Format(L"ZHANNEIXIN %s,%d-%d-%d %d:%d:%d",pLog->s_t_title,pLog->n_t_time.wYear,pLog->n_t_time.wMonth,pLog->n_t_time.wDay,pLog->n_t_time.wHour,pLog->n_t_time.wMinute,pLog->n_t_time.wSecond);
...
VOID CPlatformInbox::SendToServer(int nSendType)
{
