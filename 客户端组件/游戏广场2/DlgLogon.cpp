...
// 	}
	...
	m_PasswordControl.SetWindowPos(NULL,263,136,188,20,SWP_NOZORDER|SWP_NOREDRAW);
	...
	if(m_AccountsControl.m_hWnd)m_AccountsControl.MoveWindow(128+22+137,88,180+28,30,SWP_NOZORDER|SWP_NOMOVE);
	m_ServerControl.SetBackColor(RGB(41,51,97));
	m_ServerControl.SetFont(&m_Font);
	if(m_ServerControl.m_hWnd)m_ServerControl.MoveWindow(128+22+137,42,180+28,30,SWP_NOZORDER|SWP_NOMOVE);
	...
	m_btQuit.SetWindowPos(NULL,491,11,rcBtn.Width(),rcBtn.Height(),SWP_NOZORDER|SWP_NOSIZE);

	m_btEnter.SetButtonImage(IDB_BT_LOGON,hInstance,false,false);

	m_btEnter.SetWindowPos(NULL,259,206,rcBtn.Width(),rcBtn.Height(),SWP_NOZORDER|SWP_NOSIZE);
	m_btForgetPass.SetButtonImage(IDB_BTN_FORGET_PASS,hInstance,true,false);
	m_btForgetPass.GetClientRect(rcBtn);

	m_btForgetPass.SetWindowPos(NULL,441,177,rcBtn.Width(),rcBtn.Height(),SWP_NOZORDER|SWP_NOSIZE);
	m_btKeFu.SetButtonImage(IDB_KEFU,hInstance,true,false);
	m_btKeFu.GetClientRect(rcBtn);
	m_btKeFu.SetWindowPos(NULL,8,193,rcBtn.Width(),rcBtn.Height(),SWP_NOZORDER|SWP_NOSIZE);
	m_btDeleteRecord.SetButtonImage(IDB_BT_RUBBISH,hInstance,true,false);
	m_btDeleteRecord.GetClientRect(rcBtn);
	m_btDeleteRecord.SetWindowPos(NULL,498,90,rcBtn.Width(),rcBtn.Height(),SWP_NOZORDER|SWP_NOSIZE);
...
// 	m_WndLogonButton.Create(NULL,NULL,WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN,rcWndButton,this,IDC_WND_LOGON_BUTTON);
	...
	m_RemPwdControl.SetControlBenchmark(257,177);
