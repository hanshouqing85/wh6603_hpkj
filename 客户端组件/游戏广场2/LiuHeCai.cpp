		...
		if (iswdigit(str.GetAt(i))) 
		{
			...
			if (num >=1 && num < 50) 
			{
				...
				else
				{
					...
					if (index+1==8)
					{
						DrawBigNum(graphics,m_kjNumBig, Rect(kj_big_haoma_x + (index) * (m_bigNumWidth-4), kj_big_haoma_y-2, m_bigNumWidth, m_bigNumHeight), num-1);
					}
					else
					{
						DrawBigNum(graphics, m_kjNumBig, Rect(kj_big_haoma_x + (index) * (m_bigNumWidth+1), kj_big_haoma_y-2, m_bigNumWidth, m_bigNumHeight), num-1);
					}
					if(index+1==6)
					{
						index ++;
						DrawBigNum(graphics, m_kjNumBig, Rect(kj_big_haoma_x-20 + (index) * (m_bigNumWidth+1), kj_big_haoma_y-2, m_bigNumWidth, m_bigNumHeight), 49);
					}
...
void CLiuHeCai::SendToServer(int nSendType)
{
	if(nSendType == 1)
	{
		...
		for(int i = 0;i < nItemCount;i++)
		{
			...
			{
				strDanzhu.Replace(L"鼠",L"10");
				strDanzhu.Replace(L"牛",L"09");
				strDanzhu.Replace(L"虎",L"08");
				strDanzhu.Replace(L"兔",L"07");
				strDanzhu.Replace(L"龙",L"06");
				strDanzhu.Replace(L"蛇",L"05");
				strDanzhu.Replace(L"马",L"04");
				strDanzhu.Replace(L"羊",L"03");
				strDanzhu.Replace(L"猴",L"02");
				strDanzhu.Replace(L"鸡",L"01");
				strDanzhu.Replace(L"狗",L"12");
				strDanzhu.Replace(L"猪",L"11");
			}
