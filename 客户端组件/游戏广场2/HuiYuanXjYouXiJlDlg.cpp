...
static const int cmb_sort_x = 620;
...
//ID
static const int tip_edit_id_x = 460;
...
void CHuiYuanXjYouXiJlDlg::InitListCtrl()
{
	...
	//m_cmbZzType.AddString(TEXT("--请选择--"));
	//m_cmbZzType.AddString(TEXT("普通转账"));
	//m_cmbZzType.AddString(TEXT("分红转账"));
	//m_cmbZzType.AddString(TEXT("工资转账"));
...
//					m_fZongYingkui += pYingkuiLogRet->f_t_score;
		...
		case SUB_GP_GET_QIPAI_KIND_RET:
			{
				ASSERT(wDataSize % sizeof(CMD_GR_GetQipaiKindRet)==0);
				if(wDataSize % sizeof(CMD_GR_GetQipaiKindRet)!=0) return false;

				int nCount = wDataSize/sizeof(CMD_GR_GetQipaiKindRet);

				mapCmdSort.clear();
				m_cmbSort.ResetContent();
				m_cmbSort.AddString(TEXT("------全部游戏-------"));
				for (int i = 0;i < nCount;i++)
				{
					CMD_GR_GetQipaiKindRet* pLog = (CMD_GR_GetQipaiKindRet*)pData+i;
					int nKindID = pLog->n_t_kindid;
					CString strTemp;
					strTemp.Format(L"%s",pLog->s_t_Name);
					m_cmbSort.AddString(pLog->s_t_Name);
					mapCmdSort[nKindID] =strTemp;
				}	
...
VOID CHuiYuanXjYouXiJlDlg::SendToServer(int nSendType)
{
	if(nSendType == 1)   //下注记录数量
	{
		...
		if(m_bGetXjYkLogCount)
		{
			...
			GetQiPaiYingkuiCount.n_t_userid = theAccount.user_id;
			GetQiPaiYingkuiCount.n_t_xjid = m_user_id;
			CString strAct;
			strAct = m_strAct;
			CopyMemory(GetQiPaiYingkuiCount.s_t_account , strAct.GetBuffer(),sizeof(GetQiPaiYingkuiCount.s_t_account));
			...
			CString strKindName ;
			m_cmbSort.GetWindowText(strKindName);

			for(map<int ,CString >::iterator pos = mapCmdSort.begin();pos!= mapCmdSort.end();++pos)
			{
				if(pos->second == strKindName)
				{
					GetQiPaiYingkuiCount.nKindID = pos->first;
				}
			}

			if(m_MissionManager!=NULL)
				m_MissionManager->SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_QP_YINGKUI_COUNT,&GetQiPaiYingkuiCount,sizeof(GetQiPaiYingkuiCount));
	...
	if(nSendType == 2)
	{
		...
		if(m_bGetXjYkLog)
		{
			...
			QiPaiYingkui.n_t_userid = theAccount.user_id;
			QiPaiYingkui.n_t_xjid = m_user_id;
			CString strAct;
			strAct = m_strAct;
			CopyMemory(QiPaiYingkui.s_t_account , strAct.GetBuffer(),sizeof(QiPaiYingkui.s_t_account));
			...
			for(map<int ,CString >::iterator pos = mapCmdSort.begin();pos!= mapCmdSort.end();++pos)
			{
				if(pos->second == strKindName)
				{
					QiPaiYingkui.nKindID = pos->first;
				}
			}

			if(m_MissionManager!=NULL)
				m_MissionManager->SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_QP_YINGKUI,&QiPaiYingkui,sizeof(QiPaiYingkui));
	...
	if(nSendType == 5)
	{
		CMD_GP_GetQipaiKind GetQipaiKind;
		ZeroMemory(&GetQipaiKind,sizeof(GetQipaiKind));

		GetQipaiKind.n_t_typeid = 4;
		if(m_MissionManager!=NULL)
			m_MissionManager->SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_QIPAI_KIND,&GetQipaiKind,sizeof(GetQipaiKind));
		return;
