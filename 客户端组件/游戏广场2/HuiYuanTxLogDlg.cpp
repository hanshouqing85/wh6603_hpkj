...
// 			m_tEnd += CTimeSpan(0, 23, 59, 59);
			...
			if(m_MissionManager!=NULL)
				m_MissionManager->SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_XJTX_LOG_COUNT,&GetXjTxLogCount,sizeof(GetXjTxLogCount));
			return;
		}
	...
	if(nSendType == 2)
	{
		...
		if(m_bGetXjTxLog)
		{
			...
			if(m_MissionManager!=NULL)
				m_MissionManager->SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_XJTX_LOG,&GetXJTxLog,sizeof(GetXJTxLog));
			return;
