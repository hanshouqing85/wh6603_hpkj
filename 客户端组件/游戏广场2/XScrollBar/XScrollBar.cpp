...
//     TRACE0(AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW, 0, 0, 0));
//     TRACE0(AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW, 0, 0, 0));
//     TRACE0(AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW, 0, 0, 0));
...
//             if (m_nThumbTop > point.y) // page up
//             {
// 			    if (m_pParent && ::IsWindow(m_pParent->m_hWnd))
// 				    m_pParent->PostMessage(WM_VSCROLL, MAKELONG(SB_PAGEUP, 0), 
// 				    (LPARAM)m_hWnd);
//             }
//             else if (m_nThumbTop < point.y) // page down
//             {
//                 if (m_pParent && ::IsWindow(m_pParent->m_hWnd))
//                     m_pParent->PostMessage(WM_VSCROLL, MAKELONG(SB_PAGEDOWN, 0), 
//                     (LPARAM)m_hWnd);
//             }
...
LRESULT CXScrollBar::OnMouseHover(WPARAM wParam, LPARAM lParam)
{
    //TRACE0("Mouse hover.\n");
    return TRUE;
}

LRESULT CXScrollBar::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
    //TRACE0("Mouse leave.\n");
    return TRUE;
}
