...
BOOL  CPlazaViewItem:: OnMouseWheel(   UINT   nFlags,   short   zDelta,   CPoint   pt   )
{
	...
	if(!rcCaipiao.PtInRect(pt)&&rcQiPai.PtInRect(pt))
	{
		...
		else if(zDelta == -120)
		{
			...
			int nAllHeight = (nVCount-1)*(GAME_KIND_ITEM_CY+GAME_KIND_ITEM_PY);
			CString strLog;
			strLog.Format(L"VSCROLL si.nPos:%d,si.nMax:%d,page:%d",m_nScrollXQp/(GAME_KIND_ITEM_CY+10),nAllHeight/GAME_KIND_ITEM_CY,m_wKindPageCount);
			OutputDebugString(strLog);
...
// 			}
			...
			else
			{
				...
				if ((nYItem>=0)&&(nXItem>=0)&&(nYItem<=nAllCount)&&(nXItem<m_wKindXCount+3))
				{
					...
					{
 						CString strLog;
 						strLog.Format(L"HOVERINDEX nYItem:%d,nXItem:%d,nAllCount:%d,m_wKindXCount:%d,wResultItem:%d",nYItem,nXItem,nAllCount,m_wKindXCount,wResultItem+nCaiCount);
 						OutputDebugString(strLog);
...
// 				OutputDebugString(strLog);
			...
			CBitImage ImageGround1;
			ImageGround1.LoadFromResource(AfxGetInstanceHandle(),IDB_PLAZA_GROUND1);
			ImageGround1.DrawImageTile(pBufferDC,0,0,ImageGround1.GetWidth(),ImageGround1.GetHeight()/*rcClient.Width(),rcClient.Height()*/);
...
VOID CPlazaViewItem::DrawKindItem(CDC * pDC, WORD wViewIndex, INT nXPos, INT nYPos, tagGameKindInfo * pGameKindInfo)
{
	...
	//安装判断
	bool bInstall=false;
	if (pGameKindInfo->pGameKindItem!=NULL&&pGameKindInfo->pGameKindItem->m_dwProcessVersion!=0L) bInstall=true;
...
VOID CPlazaViewItem::OnLButtonUp(UINT nFlags, CPoint Point)
{
 ...
 //		}
	...
	//}
	//else 
	if (m_cbShowItemMode == VIEW_MODE_TYPE&&(wViewItemDown!=INVALID_WORD)&&(cbButtonDown!=0))
	{
		switch (cbButtonDown)
		{
