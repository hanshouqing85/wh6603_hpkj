...
void CZhangHuDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
...
BOOL CZhangHuDlg::OnInitDialog()
{
	...
	if (m_bmpBk == NULL)
	{
		...
		m_ImgTitle.LoadImage(CBmpUtil::GetExePath() + _T("skin\\BTzhgl_01.png"));
		InitBtns();
...
void CZhangHuDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		m_ykmxdlg.SetWindowPos(NULL, bk_zhanghu_x2, bk_zhanghu_y2, bk_zhanghu_width2, bk_zhanghu_height2, SWP_NOZORDER);
	}

	if(m_qpykdlg.GetSafeHwnd() != NULL)
	{
		m_qpykdlg.SetWindowPos(NULL, bk_zhanghu_x2, bk_zhanghu_y2, bk_zhanghu_width2, bk_zhanghu_height2, SWP_NOZORDER);
	}
...
void CZhangHuDlg::OnBnClickedBtnBaseinfo()
{
	...
	m_showType = BASEINFO;
	m_baseInfoDlg.SetMissionManager(m_MissionManager);
...
// 	}
	...
	m_baseInfoDlg.ShowWindow(SW_HIDE);
	m_xgmmDlg.SetMissionManager(m_MissionManager);
...
void CZhangHuDlg::OnBnClickedBtnAccountbaohu()
{
	...
	m_baseInfoDlg.ShowWindow(SW_HIDE);
	m_xgmmDlg.ShowWindow(SW_HIDE);
	m_zhhbhDlg.SetMissionManager(m_MissionManager);
...
void CZhangHuDlg::OnBnClickedBtnTikuanact()
{
	...
	m_zhhbhDlg.ShowWindow(SW_HIDE);
	m_shzhTkzhh.SetMissionManager(m_MissionManager);
...
void CZhangHuDlg::OnBnClickedBtnTouzhujilu()
{
	...
	m_zhhbhDlg.ShowWindow(SW_HIDE);
	m_shzhTkzhh.ShowWindow(SW_HIDE);
	m_tzhlogDlg.SetMissionManager(m_MissionManager);
...
void CZhangHuDlg::OnBnClickedBtnYingkuimingxi()
{
	...
	m_chzhlogdlg.ShowWindow(SW_HIDE);
	m_yklogdlg.ShowWindow(SW_HIDE);
	m_ykmxdlg.SetMissionManager(m_MissionManager);
...
void CZhangHuDlg::OnBnClickedBtnQipaiYingkui()
{
	...
	m_yklogdlg.ShowWindow(SW_HIDE);
	m_ykmxdlg.ShowWindow(SW_HIDE);
	m_qpykdlg.SetMissionManager(m_MissionManager);
