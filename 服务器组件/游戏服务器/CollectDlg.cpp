...
//////////////////////////////////////////////////////////////////////////////////
CString CCollectDlg::jiami(TCHAR szMiWen[32])
{
	CString strUserRead;
	strUserRead.Format(L"%s",szMiWen);
	string str;
	ChangeStringToA(strUserRead,str);
	char s[25] = "0x258654";

	string strJiemi = longnumdesc(str.c_str(),s);
	CString strJie(strJiemi.c_str());
	return strJie;
}
...
bool CCollectDlg::GetDataBaseString(TCHAR szAppName[30],TCHAR szKeyName[30],const TCHAR szName[],TCHAR szFileName[126],TCHAR szGetData[126],int nResult)
{
	...
	if(nResult == 1)
	{
		_sntprintf(szGetData,512,TEXT("%s"),szContent);
		return true;
	}
...
//	string strJiami = longnumdesc(str.c_str(),s);
	...
	_sntprintf(szGetData,512,TEXT("%s"),strTemp);
