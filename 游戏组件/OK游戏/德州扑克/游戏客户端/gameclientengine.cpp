...
LRESULT	CGameClientEngine::OnStart(WPARAM wParam, LPARAM lParam)
{
	...
	m_GameClientView.SetSmallBlindFlag(INVALID_CHAIR);
	m_GameClientView.SetBigBlindFlag(INVALID_CHAIR);
	m_GameClientView.SetCenterScore(0L);

	//发送消息
	SendUserReady(NULL,0);

	//SendSocketData(SUB_C_READY,NULL,0);// add  zrh
...
LRESULT CGameClientEngine::OnFollow(WPARAM wParam,LPARAM lParam)
{
	...
	AddScore.lScore=m_lTurnLessScore;	//最小跟注
	AddScore.cbJudgeAction = GAME_ACTION_FOLLOW;			//跟注
...
LRESULT CGameClientEngine::OnOKScore(WPARAM wParam,LPARAM lParam)
{
	...
	AddScore.cbJudgeAction=GAME_ACTION_ADD;	//加注
	SendSocketData(SUB_C_ADD_SCORE,&AddScore,sizeof(AddScore));
...
LRESULT CGameClientEngine::OnPassCard(WPARAM wParam,LPARAM lParam)
{
	...
	CMD_C_AddScore AddScore;
	AddScore.lScore=0L;
	AddScore.cbJudgeAction=GAME_ACTION_PASS; //让牌
	SendSocketData(SUB_C_ADD_SCORE,&AddScore,sizeof(AddScore));
...
LRESULT CGameClientEngine::OnShowHand(WPARAM wParam,LPARAM lParam)
{
	...
	if (ShowHandObj.DoModal()==IDOK)
	{
		...
		if(i<GAME_PLAYER)
		{
			...
			AddScore.cbJudgeAction=GAME_ACTION_ALLIN;  //梭哈
			SendSocketData(SUB_C_ADD_SCORE,&AddScore,sizeof(AddScore));
...
//	SendSocketData(SUB_C_SIT,NULL,0);// add  zrh
	return 0;
}
		...
		if(m_bReset)
		{
			for (WORD i=0;i<GAME_PLAYER;i++)
			{
				...
				if (pClientUserItem!=NULL)
				{
					...
					_sntprintf(szBuffer,CountArray(szBuffer),TEXT("\n%s：")TEXT("%0.2f"),/*%s玩家\n得分:%ld*/
						strLog/*pClientUserItem->GetNickName()*/,m_dEndScore[i]);
...
bool CGameClientEngine::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	m_wMaxChipInUser=pGameStart->wMaxChipInUser;
	...
	//设置界面,添加小盲注和大盲注
	m_GameClientView.SetDFlag(m_wDUser);
	m_GameClientView.SetSmallBlindFlag(pGameStart->wMinChipInUser);
	m_GameClientView.SetBigBlindFlag(pGameStart->wMaxChipInUser);
...
bool CGameClientEngine::OnSubAddScore(const void * pBuffer, WORD wDataSize)
{
	...
	if ((IsLookonMode()==true)||(pAddScore->wAddScoreUser!=wMeChairID))
	{
		...
		//if (m_cbPlayStatus[wAddScoreUser]==TRUE)
		//{
		//	m_GameClientView.SetUserEachScore(pAddScore->wAddScoreUser,pAddScore->lAddScoreCount);
