...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_S_GameEnd[%d] 点炮用户wProvideUser=[%d],吃胡麻将cbChiHuCard=[%x],胡牌类型wChiHuKind[0,1,2,3]=[%d,%d,%d,%d],花朵个数bFlowerCount[0,1,2,3]=[%d,%d,%d,%d],底分大小lCellScore=[%d],游戏积分lGameScore[0,1,2,3]=[%lld,%lld,%lld,%lld]"),cbReason,GameEnd.wProvideUser,GameEnd.cbChiHuCard,GameEnd.wChiHuKind[0],GameEnd.wChiHuKind[1],GameEnd.wChiHuKind[2],GameEnd.wChiHuKind[3],GameEnd.bFlowerCount[0],GameEnd.bFlowerCount[1],GameEnd.bFlowerCount[2],GameEnd.bFlowerCount[3],GameEnd.lCellScore,GameEnd.lGameScore[0],GameEnd.lGameScore[1],GameEnd.lGameScore[2],GameEnd.lGameScore[3]);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_C_OutCard wChairID=[%d],麻将数据cbCardData=[%x]"),pUserData->wChairID,pOutCard->cbCardData);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_C_OperateCard wChairID=[%d],操作代码cbOperateCode=[%d],操作麻将cbOperateCard=[%x]"),pUserData->wChairID,pOperateCard->cbOperateCode,pOperateCard->cbOperateCard);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_S_OutCard 出牌用户wOutCardUser=[%d],出牌麻将cbOutCardData=[%x]"),wChairID,cbCardData);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_S_OperateResult 操作用户wOperateUser=[%d],供应用户wProvideUser=[%d],操作代码cbOperateCode=[%d],操作麻将cbOperateCard=[%x]"),OperateResult.wOperateUser,OperateResult.wProvideUser,OperateResult.cbOperateCode,OperateResult.cbOperateCard);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif		
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_S_OperateResult杠牌操作 操作用户wOperateUser=[%d],供应用户wProvideUser=[%d],操作代码cbOperateCode=[%d],操作麻将cbOperateCard=[%x]"),OperateResult.wOperateUser,OperateResult.wProvideUser,OperateResult.cbOperateCode,OperateResult.cbOperateCard);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_S_SendCard 当前用户wCurrentUser=[%d],补花动作bRepairFlower=[%d],麻将数据cbCardData=[%x],动作掩码cbActionMask=[%d]"),SendCard.wCurrentUser,SendCard.cbCardData,SendCard.bRepairFlower,SendCard.cbActionMask);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("CMD_S_OpenDoor 骰子大小wDiceCount=[%d],门牌大小cbDoorCard=[%x],开金玩家wOpenGoldUser=[%d]"),OpenDoor.wDiceCount,OpenDoor.cbDoorCard,OpenDoor.wOpenGoldUser);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
bool CTableFrameSink::OnUserOpenGold()
{
	...
	if(cbGoldIndex<SPRING_INDEX)//金牌为筒，万，索，东，南，西，北，中，发，白中的一种
	{
		// 金牌不能是花牌,开金简化处理,一定会执行到这里<by hxh 20170309>
		for(BYTE i=0;i<4;i++)
	...
	{
		if(cbGoldIndex<SPRING_INDEX+4)//春夏秋冬为金牌
		{
			for(BYTE i=0;i<4;i++)
...
#ifdef USE_RS_PRINT
	CString strLog;
	strLog.Format(_T("游戏过程中，玩家补花 m_wCurrentUser=[%d]"),m_wCurrentUser);
	CTraceService::TraceString(strLog,TraceLevel_Normal);	
	ofstream outf(g_wcTmp,ios::app); 
	outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("游戏状态前,所有玩家进行补花 CMD_S_OutCard 出牌用户wOutCardUser=[%d],出牌麻将cbOutCardData=[%x]"),pRepairFlower->wChairID,pRepairFlower->cbCurrentCard);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
...
#ifdef USE_RS_PRINT
		CString strLog;
		strLog.Format(_T("补花结束 CMD_S_SendCard 当前用户wCurrentUser=[%d],动作掩码cbActionMask=[%d]"),SendCard.wCurrentUser,SendCard.cbActionMask);
		CTraceService::TraceString(strLog,TraceLevel_Normal);	
		ofstream outf(g_wcTmp,ios::app); 
		outf<<CT2A(strLog)<<endl;
#endif
