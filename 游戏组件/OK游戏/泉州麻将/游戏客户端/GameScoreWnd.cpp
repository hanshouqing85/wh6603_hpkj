...
void CGameScoreWnd::OnPaint()
{
	...
	BufferDC.SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont()/*CSkinAttribute::m_DefaultFont*/);
	...
	for(WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		{			
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("庄家	X %d"),m_GameScoreInfo.bGoOnBankerTime);
		...
		else if(m_GameScoreInfo.wProvideUser==i)//点炮玩家
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("点炮"));
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		if(m_GameScoreInfo.bGoldCount[i]>0)
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("金牌	 %d 翻"),m_GameScoreInfo.bGoldCount[i]);
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		if(m_GameScoreInfo.bGangCount[i]>0)
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("杠牌	 %d 翻"),m_GameScoreInfo.bGangCount[i]);
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		if(m_GameScoreInfo.bFlowerCount[i]>0)
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("花牌	 %d 翻"),m_GameScoreInfo.bFlowerCount[i]);
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		if(m_GameScoreInfo.bAllSidesCard[i]==true)
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("东西南北	 1 翻"));
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		if(m_GameScoreInfo.bAllPlantCard[i]==true)
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("梅兰竹菊	 1 翻"));
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		if(m_GameScoreInfo.bAllSeasonCard[i]==true)
		{
			rcDraw.top = 150+bRowShow*20;
			rcDraw.bottom = 150+bRowShow*20+20;
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("春夏秋冬	 1 翻"));
			BufferDC.DrawText(szBuffer,lstrlen(szBuffer),&rcDraw,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
		...
		{			
			...
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("(%d翻+%d底)X%d =%d"),bTotalPan,m_GameScoreInfo.lCellScore,m_GameScoreInfo.bGoOnBankerTime,lScore);
		...
		{
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%d翻"),bTotalPan);
		...
		_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),m_GameScoreInfo.lScore[i]);
	...
	/*for (int i=0;i<GAME_PLAYER;i++)
	{
		...
		if(m_GameScoreInfo.bFlowerCount[i]>0)
		{
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%d"),m_GameScoreInfo.bFlowerCount[i]);
			BufferDC.TextOut(270,125+i*25,szBuffer,lstrlen(szBuffer));
		}

		if(m_GameScoreInfo.bGoldCount[i]>0)
		{
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%d"),m_GameScoreInfo.bGoldCount[i]);
