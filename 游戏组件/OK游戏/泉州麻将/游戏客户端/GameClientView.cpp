...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	if(m_wDoorDice!=0)
	{
		_sntprintf(szString,sizeof(szString),TEXT("开门点数：%d"),m_wDoorDice);
	...
	if(m_wGoldDice!=0)
	{
		_sntprintf(szString,sizeof(szString),TEXT("开金点数：%d"),m_wGoldDice);
...
{	
	TCHAR str[100];	
    _sntprintf( str,CountArray(str),TEXT("听"));
	//sprintf( str, "听");
	switch(byPlayer)
	{
		case 0:
