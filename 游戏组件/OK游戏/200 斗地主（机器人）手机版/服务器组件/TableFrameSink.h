...
protected:
	...
	//BYTE							m_cbRodBankCount;					//抢地主次数
	//bool							m_bQiangCount;						//是否是最后一次

	WORD							m_wFirstUser;						//首叫用户
	WORD							m_wValidCardUser[GAME_PLAYER];		//首叫明牌
	WORD							m_wBankerUser;						//庄家用户
	WORD							m_wCurrentUser;						//当前玩家
	BYTE							m_cbOutCardCount[GAME_PLAYER];		//出牌次数
	BYTE							m_cbWaitTime;						//等待时间
...
protected:
	...
	WORD							m_wValidCardTime[GAME_PLAYER];		//明牌倍数
...
protected:
	...
	//用户抢地主
	bool OnUserRodBanker(WORD wChairID, BYTE cbRodInfo);
	//用户加倍
	//bool OnUserDouble(WORD wChairID, BYTE cbDoubleInfo);
	//用户叫地主
	bool OnUserCallBanker(WORD wChairID, BYTE cbCallInfo);
	//用户出牌
