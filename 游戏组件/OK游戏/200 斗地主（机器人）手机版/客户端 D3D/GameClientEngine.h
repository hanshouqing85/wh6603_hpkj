...
protected:
	...
	//bool							m_bValidCardInfo[GAME_PLAYER];		//明牌信息//
	...
	WORD							m_wStartTime;						//初始倍数
	//WORD							m_wValidCardTime;					//明牌倍数
	WORD							m_wRodBankerTime;					//抢地主倍数
	WORD							m_wBackCardTime;					//底牌倍数
	WORD							m_wBombTime;						//炸弹倍数
	WORD							m_wChunTianbTime;					//春天倍数
	WORD							m_wCardTypeTime;					//牌型倍数

	WORD							m_wMostCardUser;					//最大玩家
	WORD							m_wFirstValidUser;					//首叫明牌
...
protected:
	...
	//用户抢地主
	bool OnSubRodBanker(VOID * pData, WORD wDataSize);
	//用户叫分
	...
	//出牌开始
	bool OnSubOutCardStart(VOID * pData, WORD wDataSize);
	//游戏结束
...
protected:
	...
	//叫地主消息
	LRESULT OnMessageCallBanker(WPARAM wParam, LPARAM lParam);
	//抢地主消息
	LRESULT OnMessageRodBanker(WPARAM wParam, LPARAM lParam);
	//右键扑克
