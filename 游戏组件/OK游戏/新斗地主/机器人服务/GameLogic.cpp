...
VOID CGameLogic::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, tagOutCardTypeResult CardTypeResult[12+1])
{
	...
	//对牌类型
	{
		...
		GetAllDoubleCard(cbTmpCardData, cbHandCardCount-cbAllBomCardCount, cbDoubleCardData, cbDoubleCardcount) ;
	...
	//三条类型
	{
		...
		GetAllThreeCard(cbTmpCardData, cbHandCardCount-cbAllBomCardCount, cbThreeCardData, cbThreeCardCount) ;
...
VOID CGameLogic::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbTurnCardData[], BYTE const cbTurnCardCount,tagOutCardTypeResult CardTypeResult[12+1])
{
	ZeroMemory(CardTypeResult, sizeof(CardTypeResult[0])*12) ;
	//数据校验
	if(cbHandCardCount<cbTurnCardCount) return ;   //如果出牌数大于自已的跳出
	...
	BYTE cbTurnCardType = GetCardType(cbTurnCardData, cbTurnCardCount) ;		//获取出牌类型
	ASSERT(cbTurnCardType!=CT_ERROR) ;
	if(cbTurnCardType==CT_ERROR) return ;										//如果出牌类型是错误类型直接返回

	if(cbTurnCardType!=CT_MISSILE_CARD && cbTurnCardType!=CT_BOMB_CARD)			//如果上家出牌不是炸弹以上的牌
	{
		//双王炸弹
		if(cbHandCardCount>=2 && 0x4F==cbTmpCard[0] && 0x4E==cbTmpCard[1])
		{
			BYTE Index = CardTypeResult[CT_BOMB_CARD].cbCardTypeCount;
			CardTypeResult[CT_BOMB_CARD].cbCardType = CT_BOMB_CARD ;  /////////这里不是双王炸吗?怎么是普通炸?
	...
	case CT_DOUBLE:				//对牌类型
		{
			...
			for (BYTE i=0;i<cbHandCardCount-cbAllBomCardCount;i++)
			{
				...
				//搜索同牌
				for (BYTE j=i+1;j<cbHandCardCount-cbAllBomCardCount;j++)
				{
					//获取扑克
	...
	case CT_THREE_TAKE_TWO://三带一对
		{
			...
			//去除2和王
			if(cbTurnThreeCount>3)
				for(BYTE i=0 ; i<cbHandCardCount-cbAllBomCardCount ; ++i)	if(GetCardLogicValue(cbTmpCard[i])<15)	{cbFirstCard = i ; break ;}
				...
				for(BYTE i=0; i<cbHandThreeCount; i+=3)
				{
					...
					if(cbThreeLineCardCount==cbTurnThreeCount && GetCardLogicValue(cbThreeLineCard[0])>GetCardLogicValue(cbTurnThreeCard[0]))
					{
						...
						BYTE cbAllDoubleCardData[MAX_COUNT] ;
						BYTE cbAllDoubleCardCount=0 ;
						GetAllDoubleCard(cbRemainCard, cbHandCardCount-cbTurnThreeCount-cbAllBomCardCount, cbAllDoubleCardData, cbAllDoubleCardCount) ;
						...
						//利用对牌的下标做组合，再根据下标提取出对牌
						Combination(cbComCard, 0, cbComResCard, cbComResLen, cbDoubleCardIndex, cbDoubleCardCount, cbAllDoubleCardCount/2-cbAllBomCardCount, cbDoubleCardCount);
						for(BYTE i=0; i<cbComResLen; ++i)
						{
							Index = CardTypeResult[CT_THREE_TAKE_TWO].cbCardTypeCount ;
							CardTypeResult[CT_THREE_TAKE_TWO].cbCardType = CT_THREE_TAKE_TWO ;
...
/////////////////////////自已一手牌或只有两手牌时出什么牌
	...
	BYTE cbAllBomCardCount;
	GetAllLineCard(cbHandCardData, cbHandCardCount, cbLineCard, cbLineCardCount) ;   //顺子
	GetAllThreeCard(cbHandCardData, cbHandCardCount, cbThreeLineCard, cbThreeLineCardCount) ;		//三张
	GetAllDoubleCard(cbHandCardData, cbHandCardCount, cbDoubleLineCard, cbDoubleLineCount) ;		//两对
	GetAllBomCard(cbHandCardData, cbHandCardCount, cbAllBomCardData, cbAllBomCardCount) ;			//炸弹
	...
	if(cbHandCardCount>=2)
	{
		...
		BYTE cbFirstHandCardType = GetCardType(cbHandCardData, cbHandCardCount-1) ;     ////////最后一张是单张
		BYTE cbSecondHandCardType = GetCardType(cbHandCardData+1, cbHandCardCount-1) ;	///////第一张是单张

		/////////////////////////////////////如果是4带的话这下边就不执行了! 一定要改
		...
		//////////////判断上下家能不能打得过自已的牌,如果上下家都没有的话,就先出上下家都没有的牌
...
///////////////////////////////////////////////////////////////////////自已一手牌或只有两手牌时结束
////////////////////////////////////这里是普通先手出牌的  一般出牌先执行这里         //这里执行,是先出对方没有的牌
	{
		{
			//分析扑克
			...
			BYTE SouIndex=0;
			/////////////////////////////梦成网络添加!!分析自已手上扑克手数
			for(BYTE cbCardType=CT_DOUBLE; cbCardType<CT_MISSILE_CARD; ++cbCardType)
			{
			tagOutCardTypeResult const &tmpCardResult = MeOutCardTypeResult[cbCardType] ;   ///这里是已经分析过的了
				for(BYTE i=0; i<tmpCardResult.cbCardTypeCount; ++i)   ///读取当前类型的总数	
				{
				...
				SouIndex++;
								...
								//CString strLog;
								//strLog.Format(TEXT("手上还有多少手牌%d"),SouIndex);
...

		{
				...
				////////////////////////////////////梦成网络添加
				//手上只有炸弹先出最小的炸弹
				if(cbHandCardCount==cbAllBomCardCount)
				{
				//存在双王炸后减
				//计算炸弹个数
													...
													CopyMemory(OutCardResult.cbResultCard,  MeOutCardTypeResult[CT_BOMB_CARD].cbCardData[cbIndex], OutCardResult.cbCardCount) ;
													return ;
			...
			for(BYTE cbCardType=CT_DOUBLE; cbCardType<CT_BOMB_CARD; ++cbCardType)
			{
				...
				for(BYTE i=0; i<tmpCardResult.cbCardTypeCount; ++i)   ///读取当前类型的总数
				{
					...
					{
						cbThisHandCardCount = tmpCardResult.cbEachHandCardCount[i] ;   //读取当前牌形 扑克数总
						////////分析上下家的扑克
						AnalyseOutCardType(m_cbAllCardData[wUpsideOfBanker], m_cbUserCardCount[wUpsideOfBanker],tmpCardResult.cbCardData[i], tmpCardResult.cbEachHandCardCount[i] ,UpsideOutCardTypeResult) ;
						AnalyseOutCardType(m_cbAllCardData[wUndersideOfBanker], m_cbUserCardCount[wUndersideOfBanker],tmpCardResult.cbCardData[i], tmpCardResult.cbEachHandCardCount[i] ,UndersideOutCardTypeResult) ;
					}
					BYTE cbMaxValue=0 ; 
					BYTE Index = 0 ;

					//当前类形扑克,比方一对,三条,上下家如果也有的话就跳出本FOR进入下一次FOR
					//敌方可以压住牌
					...
					//////////////////这里是,这次FOR的类形,上下家都没有了...但是可能会是很大的类形喔..?!
					////判断当前类型出牌中是否存在大牌
					//是否有大牌 
					if(tmpCardResult.cbEachHandCardCount[i] != cbHandCardCount)    //不是一手牌,还有更多牌没有出
					{
						bool bHaveLargeCard=false ;
						for(BYTE j=0; j<tmpCardResult.cbEachHandCardCount[i]; ++j)
						{
							if(GetCardLogicValue(tmpCardResult.cbCardData[i][j])>=15) bHaveLargeCard=true ;  //存在大牌
							//全等于A 并且不是单连或双连的话
						...
						if(bHaveLargeCard) continue ; //跳出本次FOR
					}
					/////////////////////////////////////////////////////////查找当前类型是否有大牌,如何没有大牌就进入下一句
					...
					for(BYTE j=0; j<4; ++j)
					{
						if(cbMinSingleCardCount[j]>=cbTmpCount)
						{
							...
							cbIndex[j] = i ;
							cbOutcardType[j] = cbCardType ;   //出牌类型等于当前类型
							break ;
						}
					}
					...
					{
						//最小牌型
						cbMinCardType = cbCardType ;   //出牌类型等于当前类型
						//最小牌型中的最小单牌
			...
			if(cbMinSingleCountInFour!=MAX_COUNT)
			{
				...
				else
				{
					...
					//CString strLog;
					//strLog.Format(TEXT("这里是普通出牌的,需要改啊!太2了"));
					//CTraceService::TraceString(strLog,TraceLevel_Info);  //输出信息
					////这里是出最小类形的扑克的
					//设置变量
...
VOID CGameLogic::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult) 
{
	...
	//出牌类型
	BYTE cbOutCardType = GetCardType(cbTurnCardData,cbTurnCardCount) ;    /////////获取上家出牌的类形
	AnalyseOutCardType(cbHandCardData,cbHandCardCount,cbTurnCardData,cbTurnCardCount, CardTypeResult) ;  //分析自已扑克 获取比出牌用户大的同类牌形
	...
	//梦成网络添加
	{
									BYTE DanIndex=AnalyseSinleCardCount(cbHandCardData, cbHandCardCount, NULL, 0);					//单牌个数

									//炸弹个数与单牌+1个数相同的话就炸掉对方的扑克
									if(DanIndex+1<=CardTypeResult[CT_BOMB_CARD].cbCardTypeCount)
									{
	...
	//可出扑克（这里已经过滤掉炸弹了）
	tagOutCardTypeResult const &CanOutCard = CardTypeResult[cbOutCardType] ;    //获取用户出牌类型中自已的扑克
	...
	if(CanOutCard.cbCardTypeCount>0)
	{
		...
		if(bFindCard)
		{
			...
			if(wOutCardUser==wUndersideUser) cbLargestLogicCard = GetCardLogicValue(m_cbAllCardData[wUndersideUser][0]) ;  //获取下家最大扑克数据值
			else if(wOutCardUser==wUpsideUser) cbLargestLogicCard = GetCardLogicValue(m_cbAllCardData[wUpsideUser][0]) ;   //获取上家最大扑克数据值
			bool bCanOut=true ;
			...
			////////////梦成网络添加
			//双王判断
			if(GetCardLogicValue(CanOutCard.cbCardData[cbCanOutIndex][0])>15 && cbHandCardCount>=2 && cbHandCardData[0]==0x4F && cbHandCardData[1]==0x4E)
			{
			...
			///不诉双皇
			if(cbHandCardData[0]==0x4F && cbHandCardData[1]==0x4E && GetCardColor(CanOutCard.cbCardData[cbCanOutIndex][0])==0x40) return ;
			/////////////////////梦成网络添加
			//不能拆炸
			/////////////////////////////////如果对家牌比较多,或者自已牌比较多时,不打对2
			///应该还判断自已还有多少手牌
			//如果对方扑克数大于10并且自已扑克大于8时不用对2顶
			if(GetCardLogicValue(CanOutCard.cbCardData[cbCanOutIndex][0])>=15 && m_cbUserCardCount[wOutCardUser]>=8 && cbHandCardCount>8 &&  cbOutCardType!=CT_SINGLE) 								
			{
								//CString strLog;
								//strLog.Format(TEXT("跳出1"));
								//CTraceService::TraceString(strLog,TraceLevel_Info);  //输出信息
					return ;
			}
		...
		for(BYTE i=0; i<CardTypeResult[cbOutCardType].cbCardTypeCount; ++i)
		{
			...
			///应该还判断自已还有多少手牌
			//如果对方扑克数大于10并且自已扑克大于6时不用对2顶
			if(GetCardLogicValue(CardTypeResult[cbOutCardType].cbCardData[Index][0])>=15 && m_cbUserCardCount[wOutCardUser]>=8 && cbHandCardCount>8 && cbOutCardType!=CT_SINGLE) 								
			{
								//CString strLog;
								//strLog.Format(TEXT("跳出2"));
								//CTraceService::TraceString(strLog,TraceLevel_Info);  //输出信息
					return ;
			}
								...
								//CString strLog;
								//strLog.Format(TEXT("这里有问题,需要改2"));
								//CTraceService::TraceString(strLog,TraceLevel_Info);  //输出信息

		//设置变量
		OutCardResult.cbCardCount=CardTypeResult[cbOutCardType].cbEachHandCardCount[Index];   //能出的类型
...
//叫分判断
BYTE CGameLogic::LandScore(WORD wMeChairID, BYTE cbCurrentLandScore,WORD AndroidUserID)
{
	//大牌数目
	...
	while(GetCardLogicValue(m_cbLandScoreCardData[Index++])>=15) ++cbLargeCardCount ;
		...
		BYTE cbTmpCardData[MAX_COUNT] ;
		//保留扑克，防止分析时改变扑克
		CopyMemory(cbTmpCardData, m_cbLandScoreCardData, sizeof(m_cbLandScoreCardData)) ;	
		SortCardList(cbTmpCardData, sizeof(cbTmpCardData), ST_ORDER) ;
		//移除炸弹
		BYTE cbAllBomCardData[MAX_COUNT] ;
		BYTE cbAllBomCardCount=0 ;
		GetAllBomCard(cbTmpCardData, sizeof(cbTmpCardData), cbAllBomCardData, cbAllBomCardCount) ;
		RemoveCard(cbAllBomCardData, cbAllBomCardCount, cbTmpCardData, sizeof(cbTmpCardData)) ;
		//分扑三张
		BYTE cbThreeCardData[MAX_COUNT];
		BYTE cbThreeCardCount=0 ;
		GetAllThreeCard(cbTmpCardData, sizeof(m_cbLandScoreCardData)-cbAllBomCardCount, cbThreeCardData, cbThreeCardCount) ;
		//多少个三张就去掉多少个单牌

		if(cbSingleCardCount>0 && cbSingleCardCount-(cbThreeCardCount/3)>0){
		
			cbSingleCardCount=cbSingleCardCount-(cbThreeCardCount/3);
		
		}

		//////////获取炸弹个数
		//cbAllBomCardCount=cbAllBomCardCount/4;

		//双王数目
		BYTE cbLargeHCount = 0 ;
		BYTE Jndex=0 ;
		while(GetCardLogicValue(m_cbLandScoreCardData[Jndex++])>15) ++cbLargeHCount ;


	//好牌玩家是当前玩家
	//叫三分
	if(wMeChairID==AndroidUserID) return 3;

	//叫三分
	if(cbLargeCardCount>=4 && cbSingleCardCount<=2) return 3 ;

	//叫三分
	if(cbLargeCardCount>=3 && cbSingleCardCount<=1 && cbAllBomCardCount>=4) return 3;

	//叫三分
	if(cbLargeHCount>=1 && cbLargeCardCount>=3 && cbSingleCardCount<=1 && cbAllBomCardCount>=4) return 3;

	//叫两分
	if(cbLargeHCount>=2 && cbLargeCardCount>=3 && cbSingleCardCount<=3) return 2;

	//叫两分
	if(cbLargeCardCount>=2 && cbSingleCardCount<=2 && cbAllBomCardCount>=4) return 2;
