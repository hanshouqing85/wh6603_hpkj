...
CAndroidUserItemSink::CAndroidUserItemSink()
{
	...
	m_wAndroidUserID=INVALID_CHAIR;
	//扑克变量
	m_cbTurnCardCount=0;
	...
	case IDI_CALL_SCORE:	//用户叫分
		{
			...
			//设置变量
						//设置变量
			CallScore.cbCallScore=m_GameLogic.LandScore(m_pIAndroidUserItem->GetChairID(),m_cbCurrentLandScore, m_wAndroidUserID);
...
bool CAndroidUserItemSink::OnSubGameStart(VOID * pData, WORD wDataSize)
{
	...
	//好牌用户ID
	m_wAndroidUserID=pAndroidCard->wAndroidUserID;
...
void CAndroidUserItemSink::ReadConfigInformation()
{
	...
	_sntprintf(szConfigFileName,sizeof(szConfigFileName),TEXT("%s\\LandConfig.ini"),szPath);
