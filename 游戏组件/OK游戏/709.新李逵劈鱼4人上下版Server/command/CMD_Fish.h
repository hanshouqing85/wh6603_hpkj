...
#endif
const float kChairDefaultAngle[GAME_PLAYER] = {M_PI, M_PI, 0, 0};				/*椅子角度转换*/
...
struct CMD_S_FireHarvest
{
	...
	__int64 iNowScore;	//当前身上的金币
	__int64 iNowFishScore;	//当前身上的鱼币
	__int64 iCanExchangeGold;	//身上的鱼币可转换成的金币
};
...
#define SUB_MOBILE_GET_Harvest				12		//手机端获取战果
#define SUB_C_ONLINE_AWARD_GET				13		//游戏在线奖励
#define SUB_C_ONLINE_AWARD_COM				14		//确认领取游戏在线奖励

struct CMD_C_ExchangeFishScore {
  int increase;		//0表示下分，1表示按配置文件设定数额上分，2表示一次性上完所有的分，3表示手机端上分
  __int64 iExchangeCount;	//手机端上分数量
};
...
{	
	...
	DBR_GR_FISH_RECORD_LOGINTIME,				//记下登录时间
