...
private:
  ...
  int lockfishpercent;
  int lostlockfishpercent;

  BulletKind current_bullet_kind_;
  int current_bullet_mulriple_;
  int lase_lock_fishid_;
  double current_bullet_angele;											//炮管角度
  bool allow_fire_;
  ...
  int firetime_android;
  int fishtime_android;
  int max_bullet_multiple_android;     //同一时刻最多发射子弹数
...
private:
  void ExchangeFishScore(bool increase);
  void Fire();
