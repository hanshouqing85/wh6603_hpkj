	...
	if( !m_bMoving || m_nXDistance > 0 )
	{
		for (WORD i=0;i<m_wCardCount;i++)
		{
			...
			if(m_bIsExtractor)
				...
				m_ImageCard.BlendDrawImage(pDC,OriginPoint.x+DEF_X_DISTANCE*i,OriginPoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255),255);
				//m_ImageCard.AlphaDrawImage(pDC,OriginPoint.x+DEF_X_DISTANCE*i,OriginPoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
...
//		ASSERT( cbCardData != 0 );
		for(WORD iLook=0; iLook<m_wCardCount ; iLook++)
		{
			cbCardData=m_CardItem[iLook].cbCardData;
			if(cbCardData!= 0)
			{
				nXImagePos=((cbCardData&CARD_MASK_VALUE)-1)*m_CardSize.cx;
				nYImagePos=((cbCardData&CARD_MASK_COLOR)>>4)*m_CardSize.cy;
			...
			else
			{
				...
				if(m_cbShowIndex == iLook)
				{
					INT nYMove = __min(m_nYMove,33);
					//m_ImageCard.AlphaDrawImage( pDC,OriginPoint.x,OriginPoint.y-nYMove/*m_CardSize.cy*3/5*/,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
					m_ImageCard.BlendDrawImage(pDC,OriginPoint.x+m_nXDistance*iLook,OriginPoint.y-nYMove,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255),255);
				}
				//nXImagePos=m_CardSize.cx*2;
				//nYImagePos=m_CardSize.cy*4;
				else 
					m_ImageCard.BlendDrawImage( pDC,OriginPoint.x+DEF_X_DISTANCE*iLook,OriginPoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255),255);
					//m_ImageCard.AlphaDrawImage( pDC,OriginPoint.x+DEF_X_DISTANCE*iLook,OriginPoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
...
VOID CCardControl::GetControlSize(CSize & ControlSize)
{
	...
	if(m_bIsExtractor)
		...
		ControlSize.cx=(m_wCardCount>0)?(m_CardSize.cx+(m_wCardCount-1)*DEF_X_DISTANCE):0;
	return;
}
...
int CCardControl::getCardCount(void)
{
	return (int)m_wCardCount;
}

BYTE CCardControl::GetLeftMouseDownCard(CPoint Point)
{
	//光标处理
	if ((m_bPositively==true)&&!m_bMoving)
	{
		...
		if( wHoverItem != INVALID_ITEM )
		{
			return m_CardItem[wHoverItem].cbCardData;
		}
	}
