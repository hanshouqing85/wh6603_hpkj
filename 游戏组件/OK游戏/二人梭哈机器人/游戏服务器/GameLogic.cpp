...
void CGameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	...
	do
	{
		cbPosition=(BYTE)(rand()%(CountArray(cbCardData)-cbRandCount));
...
//对比扑克，自己全部牌，对方明牌
int CGameLogic::CompareCard(BYTE cbFirstCardData[], BYTE cbCardCountFirst, BYTE cbNextCardData[], BYTE cbCardCountNext)
{
	//获取类型
	BYTE cbFirstGenre=GetCardGenre(cbFirstCardData,cbCardCountFirst);
	BYTE cbNextGenre=GetCardGenre(cbNextCardData,cbCardCountNext);
	//类型判断
	...
	switch(cbFirstGenre)
	{
		case CT_SINGLE:			//单牌
		{
			//对比数值
			BYTE cbFirstValue = 0;
			for (BYTE i=0;i<cbCardCountFirst;i++)
			{
				cbFirstValue=GetCardLogicValue(cbFirstCardData[i]);
			}
			BYTE cbNextValue=0;
			for(BYTE i=0; i<cbCardCountNext; i++)
			{
				cbNextValue=GetCardLogicValue(cbNextCardData[i]);
			}
			if (cbFirstValue!=cbNextValue) 
				return cbFirstValue>cbNextValue;
			//需要考虑4张牌时，头尾都可以组成顺子的情况。
		...
		case CT_THREE_TIAO:		//三条:	
		{
			...
			tagAnalyseResult AnalyseResultFirst;
			tagAnalyseResult AnalyseResultNext;
			AnalysebCardData(cbFirstCardData,cbCardCountFirst,AnalyseResultFirst);
			AnalysebCardData(cbNextCardData,cbCardCountNext,AnalyseResultNext);
			//四条数值
			if (AnalyseResultFirst.cbFourCount>0)
			{
				BYTE cbNextValue=AnalyseResultNext.cbFourLogicVolue[0];
