	...
	case GER_NORMAL:		//常规结束
		{
			...
			for (WORD i=0;i<GAME_PLAYER;i++)
			{
				...
				{
					GameEnd.lGameScore[i]=ScoreInfoArray[i].lScore-GameEnd.lGameScore[i]*m_pGameServiceOption->cbRevenueRatio/100L;
					ScoreInfoArray[i].lRevenue=GameEnd.lGameScore[i]*m_pGameServiceOption->cbRevenueRatio/100L;
				}

				//设置胜负
				if (ScoreInfoArray[i].lScore==0L) ScoreInfoArray[i].cbType=SCORE_TYPE_DRAW;
				else ScoreInfoArray[i].cbType=(ScoreInfoArray[i].lScore>0L)?SCORE_TYPE_WIN:SCORE_TYPE_LOSE;
			...
			{
				//写入积分
				m_pITableFrame->WriteUserScore(i,ScoreInfoArray[i]/*.lScore,ScoreInfoArray[i].lRevenue,ScoreInfoArray[i].cbType*/);
			}
			//结束游戏
	...
	case GER_USER_LEAVE:		//用户强退
		{
			...
			//写入积分
			m_pITableFrame->WriteUserScore(wChairID,ScoreInfoArray[wChairID]/*.lScore,0L,ScoreInfoArray[wChairID].cbType*/);
