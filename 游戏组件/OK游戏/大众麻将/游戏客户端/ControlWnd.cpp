...
CControlWnd::CControlWnd()
{
    ...
    ///m_pSinkWnd=(CGameFrameViewGDI *)AfxGetMainWnd();///
...
{
	m_pSinkWnd->PostEngineMessage(IDM_CARD_OPERATE,WIK_CHI_HU,0);
	return;
}
...
{
	m_pSinkWnd->PostEngineMessage(IDM_LISTEN_CARD,0,0);
	return;
}
...
{
	m_pSinkWnd->PostEngineMessage(IDM_CARD_OPERATE,WIK_NULL,0);
	return;
}
...
int CControlWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btListen.SetButtonImage(IDB_BT_LISTEN,AfxGetInstanceHandle(),false,false);
...
void CControlWnd::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_cbCurrentItem!=0xFF)
	{
		...
		for (BYTE i=0;i<CountArray(cbItemKind);i++)
		{
			...
			{
				m_pSinkWnd->PostEngineMessage(IDM_CARD_OPERATE,cbItemKind[i],m_cbCenterCard);
				return;
			}
		}
		...
		for (BYTE i=0;i<CountArray(m_cbGangCard);i++)
		{
			...
			{
				m_pSinkWnd->PostEngineMessage(IDM_CARD_OPERATE,WIK_GANG,m_cbGangCard[i]);
				return;
			}
		}
