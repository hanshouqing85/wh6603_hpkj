...
//游戏场景
bool CGameClientEngine::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, void * pBuffer, WORD wDataSize)
{
	switch (cbGameStatus)
	{
...
bool CGameClientEngine::OnSubTrustee(const void * pBuffer,WORD wDataSize)
{
	...
	{
		IClientUserItem * pUserData=m_GameClientView.GetClientUserItem(pTrustee->wChairID);
		TCHAR szBuffer[256];
...
void CGameClientEngine::PlayCardSound(WORD wChairID, BYTE cbCardData)
{
	...
	bool bBoy = ((pUserData->GetGender() == 1) ? false : true);
