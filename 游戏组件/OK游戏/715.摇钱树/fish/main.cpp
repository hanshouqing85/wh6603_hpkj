...
#define DEF_SCREEN_WIDTH	  1024		// 默认屏宽
...
int WINAPI _tWinMain(HINSTANCE, HINSTANCE, LPTSTR cmd_line, int) 
{
  ...
  hge_->System_SetState(HGE_LOGFILE, "zyqs.log");
  ...
  hge_->System_SetState(HGE_TITLE, "真·摇钱树");
  ...
  if (hge_->System_Initiate()) {
    ...
    DWORD size;
    void* data;
    //data = hge_->Resource_Load("zyqs\\images\\logo.fish", &size);
	data = hge_->Resource_Load("zyqs\\images\\logo.png", &size);
    ...
    //data = hge_->Resource_Load("zyqs\\images\\loading_bar.fish", &size);
	data = hge_->Resource_Load("zyqs\\images\\loading_bar.png", &size);
