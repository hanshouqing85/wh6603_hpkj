  ...
  switch (game_status) {
    ...
    case GAME_STATUS_PLAY:
      ...
      for (WORD i = 0; i < GAME_PLAYER; ++i) {
		  TCHAR tempStr[126];
		  //tempStr.Format(L"YAOQIANSHU %d,%I64d",exchange_fishscore->chair_id,exchange_fishscore->swap_fish_score);
		  _sntprintf(tempStr,126,L"YAOQIANSHU %d,%I64d",i,gamestatus->fish_score[i]);
...
void GameManager::OnEventUserLeave(IGameUserItem* game_user_item, bool lookon_user) {
  ...
  cannon_manager_->SetCannonMulriple(game_user_item->GetChairID(),min_bullet_multiple_);
}
...
bool GameManager::OnFrame(float delta_time) 
{
	...
	if (me_chair_id < GAME_PLAYER) 
	{
		...
		else if (hge_->Input_KeyDown(HGEK_S)) 
		{
			...
			lock_fish_manager_->SetLockFishID(me_chair_id, fish_manager_->LockFish(&lock_fish_kind, lock_fish_manager_->GetLockFishID(me_chair_id),lock_fish_manager_->GetLockFishKind(me_chair_id)));
	...
	if (allow_fire_ && me_chair_id < GAME_PLAYER && !client_kernel_->IsLookonMode()) 
	{
		...
		else if (hge_->Input_KeyDown(HGEK_DOWN)) 
		{
		...
		{
			SendExchangeFishScore(0);
		} 
		else if (hge_->Input_KeyDown(HGEK_RIGHT)) 
		{
			last_exchange_score_=now_time;
			SendExchangeFishScore(1);
		}
		else if (hge_->Input_GetKeyState(HGEK_RIGHT)) 
		{
			if (now_time - last_exchange_score_ >= 1000) 
			{
				SendExchangeFishScore(2);
				last_exchange_score_ = now_time;
			}
		}
		...
		if (game_active_ && can_fire && (hge_->Input_GetKeyState(HGEK_LBUTTON) || hge_->Input_GetKeyState(HGEK_SPACE) || hge_->Input_GetChar() == ' ')) 
		{
...
void GameManager::SendExchangeFishScore(int increase) 
{
  ...
  SCORE me_fish_score = cannon_manager_->GetFishScore(me_chair_id);
  SCORE need_user_score = exchange_ratio_userscore_ * exchange_count_ / exchange_ratio_fishscore_;		//兑换指定数量的鱼币所需要的金币
  ...
  if (increase == 1) 
  {
    ...
    cannon_manager_->SetFishScore(me_chair_id, need_user_score);
    exchange_fish_score_[me_chair_id] += need_user_score;
  }
  else if (increase==2)
  {
	  if (need_user_score > user_leave_score) 
	  ...
	  need_user_score = user_leave_score *exchange_ratio_fishscore_/exchange_ratio_userscore_;	//一次性上完所有的分
...
bool GameManager::OnSubExchangeFishScore(void* data, WORD data_size) {
  ...
  TCHAR tempStr[126];
  //tempStr.Format(L"YAOQIANSHU %d,%I64d",exchange_fishscore->chair_id,exchange_fishscore->swap_fish_score);
  _sntprintf(tempStr,126,L"YAOQIANSHU %d,%I64d",exchange_fishscore->chair_id,exchange_fishscore->swap_fish_score);
  ::OutputDebugString(tempStr);
...
bool GameManager::OnSubUserFire(void* data, WORD data_size) 
{
	...
	if (user_fire->chair_id != client_kernel_->GetMeChairID()) 
	{
		...
		SoundManager::GetInstance().PlayGameEffect(FIRE);
		cannon_manager_->Switch(user_fire->chair_id, user_fire->bullet_kind);
	}
...
void GameManager::RenderUserScoreNum(SCORE num, float x, float y, float rot, float hscale, float vscale) {
  ...
  int num_count = 0;
  LONGLONG num_temp = (LONGLONG)num;
  do {
    num_count++;
    num_temp /= 10;
  } while (num_temp > 0);
  ...
  LONGLONG lOldnum = (LONGLONG)num;

  for (int i = 0; i < num_count; ++i) {
	  LONGLONG fnum = lOldnum;
	  fnum/=10;
	  fnum*=10;

	  LONGLONG lNum = lOldnum-fnum;
	 // fnum -= (num/10);
    spr_user_score_num_->SetFrame(static_cast<int>(lNum));
