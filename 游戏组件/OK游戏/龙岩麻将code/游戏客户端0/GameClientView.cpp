...
#define MOVE_STEP_COUNT					1								//移动步数
#define TIME_MOVE_CARD					1								//牌动画定时时间
...
CGameClientView::CGameClientView()
{
	...
	m_ImageCenter.LoadFromResource(AfxGetInstanceHandle(),TEXT("VIEW_CENTER"));
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	for (WORD i=0;i<4;i++)
	{
		...
		m_ScoreControl[i].Create( IDD_GAME_SCORE,this );
...
//	m_ControlWnd.SetBenchmarkPos(nWidth/2+298,nHeight-BORDER_CY-120);
	...
	//移动调整
	DeferWindowPos(hDwp,m_btStart,NULL,(nWidth-rcButton.Width())/2,nHeight-BORDER_CY-155,0,0,uFlags);
	//移动调整
	...
	CRect rcGameScore;
	m_ScoreControl[0].GetWindowRect(&rcGameScore);
	CPoint ptPos( (nWidth-rcGameScore.Width())/2,(nHeight-rcGameScore.Height())*2/5-30 );
	ClientToScreen( &ptPos );
	m_ScoreControl[0].SetWindowPos( NULL,ptPos.x,ptPos.y,0,0,SWP_NOSIZE|SWP_NOACTIVATE|SWP_NOCOPYBITS );
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	if(m_cbLandCount>0)
	{
		//加载资源
		INT nY=-35;
		CPngImage ImageLaoZhuang;
		ImageLaoZhuang.LoadImage(AfxGetInstanceHandle(),TEXT("LAO_ZHUANG"));
		ImageLaoZhuang.DrawImage( pDC,BORDER_CX+10,BORDER_CY+m_PngFengQuan.GetHeight()/4+15+nY);

		//创建字体
		CFont ViewFont;
		ViewFont.CreateFont(-13,0,0,0,700,0,0,0,134,3,2,1,2,TEXT("宋体"));
		CFont *pOldFont=pDC->SelectObject(&ViewFont);

		//绘画信息
		TCHAR szLandCount[32],szCellScore[32];
		_sntprintf(szLandCount,sizeof(szLandCount),TEXT("%d"),m_cbLandCount);
		DrawTextString(pDC,szLandCount,RGB(8,206,180),RGB(0,0,0),BORDER_CX+18+ImageLaoZhuang.GetWidth(),BORDER_CY+m_PngFengQuan.GetHeight()/4+19+nY);
		_sntprintf(szCellScore,sizeof(szCellScore),TEXT("%ld"),m_lCellScore);
		DrawTextString(pDC,szCellScore,RGB(8,206,180),RGB(0,0,0),BORDER_CX+18+ImageLaoZhuang.GetWidth(),BORDER_CY+m_PngFengQuan.GetHeight()/4+42+nY);
...
void CGameClientView::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	////test
	//BYTE byCard[] = {
	//	0x01,0x02,0x37,0x04,0x05,0x06,0x07,0x08,0x37,					
	//	0x01,0x02,0x03,0x37,0x05,0x06,0x07,0x08,0x09,
	//	0x01,0x02,0x03,0x04,0x37,0x06,0x07,0x08,0x09
	//};
	//BYTE byCardCount = 14;
	//static int n = 0;
	//if( ++n==1 )
	...
	//		m_HeapCard[i].SetCardData( 10*2,0x01 );
	...
	//	{
	//		CString dd;
	//		dd.Format(TEXT("用户名%d"),i);
	//		lstrcpy( ScoreInfo.szUserName[i],dd);
	//		ScoreInfo.lGameScore[i] = 123L*((i%2)?-1:1);
	//		ScoreInfo.lHuScore[i]= -122;
	//	}
	...
	//ScoreInfo.wLandUser = 0;
	//ScoreInfo.lCellScore = 3;
	//ScoreInfo.cbLandCount = 1;
...
void CGameClientView::OnTimer(UINT nIDEvent)
{
	...
	else if( nIDEvent == IDI_MOVE_CARD )
	{
		...
		if( m_nStepCount == 0 )
		{
			...
			//发送动画结束消息
			AfxGetMainWnd()->PostMessage( IDM_MOVE_CARD_FINISH );
...
//牢庄次数
void CGameClientView::SetLandCount( BYTE cbLandCount )
{
	if( m_cbLandCount != cbLandCount )
	{
		m_cbLandCount = cbLandCount;
		InvalidGameView( 0,0,0,0 );
	}
	return;
}
