...
void CDrawSiceWnd::OnTimer(UINT nIDEvent)
{
	if(IDI_SICE_POS==nIDEvent)
	{
		if(m_bSicePos>=17)
		{
			...
			if( m_pSinkWnd )
			{
				if(m_wSice2!=0 && m_wSice3!=0)
				{
					m_pSinkWnd->PostMessage(IDM_LAND_SICE_FINISH,NULL,NULL);
				}
				else if(m_wSice1!=0 && m_wSice2!=0)
				{
					m_pSinkWnd->PostMessage(IDM_SICE_FINISH,NULL,NULL);
				}
				else
				{
					m_pSinkWnd->PostMessage(IDM_CAI_SHEN_SICE_FINISH,NULL,NULL);
				}
			}
		}
		else
...
void CDrawSiceWnd::StopSicing()
{
	if( IsWindowVisible() )
	{
		...
		if( m_bSicePos <= 8 )
			m_pSinkWnd->SendMessage( IDM_SICE_TWO );
		...
		{
			m_pSinkWnd->SendMessage(IDM_SICE_FINISH,NULL,NULL);
		}
		else
		{
			m_pSinkWnd->SendMessage(IDM_CAI_SHEN_SICE_FINISH,NULL,NULL);
		}
	}
...
//设置色子信息
void CDrawSiceWnd::SetSiceInfo(CDC *dc,BYTE bMoveTime,WORD wSice1,WORD wSice2,WORD wSice3)
{
	HINSTANCE hInstance=AfxGetInstanceHandle();
	ParentDC=dc;
	m_ImageBack.LoadFromResource(hInstance, IDB_SICE_BACK);
