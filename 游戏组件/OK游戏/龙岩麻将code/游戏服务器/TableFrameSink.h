...
protected:
	...
	BYTE							m_cbHuaCardData[GAME_PLAYER][8];		//花牌数据	 已出
	BYTE							m_cbDone[GAME_PLAYER];
	//单元积分
	LONGLONG						m_lCellScore;							//基本分
	WORD							m_wReplaceUser;							//补牌用户
	bool							m_bMustWanTong;							//是否必须
	//连庄信息
	BYTE							m_cbBankerContinuCount[4];				//连庄数目
	BYTE							m_cbHu;									//19刻


	BYTE							m_cbEnjoinCardData[GAME_PLAYER][GAME_PLAYER];	//禁止出牌
	BYTE							m_cbEnjoinCardCount[GAME_PLAYER];		//禁止出牌
	BYTE							m_cbEnjoinHuCard[GAME_PLAYER];			//禁止胡牌
	BYTE							m_cbEnjoinPengCard[GAME_PLAYER];		//禁止碰牌
...
protected:
	...
	//检查包关系
	BYTE IsAfford(WORD wUser2);
	//算分
	void GetNoWinScore(WORD wWinner,LONGLONG lScore[GAME_PLAYER],LONGLONG lHu[GAME_PLAYER]);
	//获取等级
	LONGLONG GetMemberLevelScore(LONGLONG lMinScore);
	///////////////////////////
    bool isUseTuoGuan();
	//发送托管
