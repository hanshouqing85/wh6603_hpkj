...
CMySkinScrollBar::CMySkinScrollBar()
{
	m_hBmp=NULL;
	m_bDrag=FALSE;
...
BEGIN_MESSAGE_MAP(CMySkinScrollBar, CScrollBar)
	ON_WM_SIZE()
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
...
BOOL CMySkinScrollBar::IsVertical()
{
	DWORD dwStyle=GetStyle();
	return dwStyle&SBS_VERT;
}

UINT CMySkinScrollBar::HitTest(CPoint pt)
{
	int nTestPos=pt.y;
...
void CMySkinScrollBar::SetBitmap(HBITMAP hBmp)
{
	ASSERT(m_hWnd);
	m_hBmp=hBmp;
	BITMAP bm;
...
void CMySkinScrollBar::OnSize(UINT nType, int cx, int cy) 
{
	CScrollBar::OnSize(nType, cx, cy);
	m_nHei=IsVertical()?cy:cx;
}

void CMySkinScrollBar::OnPaint() 
{
...
RECT CMySkinScrollBar::GetImageRect(UINT uSBCode,int nState)
{
	int nIndex=0;
	switch(uSBCode)
	{
...
RECT CMySkinScrollBar::GetRect(UINT uSBCode)
{
	SCROLLINFO si=m_si;
	if(si.nTrackPos==-1) si.nTrackPos=si.nPos;
	int nInterHei=m_nHei-2*m_nWid;
...
void CMySkinScrollBar::TileBlt(CDC *pDestDC, RECT *pDestRect, CDC *pSourDC, RECT *pSourRect)
{
...
void CMySkinScrollBar::DrawThumb(CDC *pDestDC, RECT *pDestRect, CDC *pSourDC, RECT *pSourRect)
{
...
void CMySkinScrollBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	SetCapture();
...
void CMySkinScrollBar::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CMySkinScrollBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
	ReleaseCapture();
	if(m_bDrag)
	{
...
void CMySkinScrollBar::OnTimer(UINT nIDEvent) 
{
...
void CMySkinScrollBar::DrawArrow(UINT uArrow, int nState)
{
...
LRESULT CMySkinScrollBar::OnMouseLeave(WPARAM wparam, LPARAM lparam)
{
	m_bTrace = FALSE;
...
LRESULT CMySkinScrollBar::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
