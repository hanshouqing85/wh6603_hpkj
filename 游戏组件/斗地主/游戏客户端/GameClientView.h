...
//#define IDM_CALL_BANKER				(WM_USER+209)						//叫地主消息
//#define IDM_ROD_BANKER				(WM_USER+210)						//抢地主消息
#define IDM_START					(WM_USER+200)						//开始消息
#define IDM_VALID_START				(WM_USER+201)						//明牌开始
#define IDM_OUT_CARD				(WM_USER+202)						//出牌消息
#define IDM_PASS_CARD				(WM_USER+203)						//PASS消息
#define IDM_OUT_PROMPT				(WM_USER+204)						//提示消息
//#define IDM_VALID_CARD				(WM_USER+205)						//明牌消息
#define IDM_CALL_BANKER				(WM_USER+206)						//叫地主消息
#define IDM_ROD_BANKER				(WM_USER+207)						//抢地主消息
#define IDM_DOUBLE					(WM_USER+208)						//加倍消息
#define	IDM_LEFT_HIT_CARD			(WM_USER+209)						//左击扑克
#define IDM_SORT_HAND_CARD			(WM_USER+210)						//排列扑克
#define IDM_LAST_TURN_CARD			(WM_USER+211)						//上轮扑克
#define IDM_TRUSTEE_CONTROL			(WM_USER+212)						//托管控制
...
protected:
	...
	LONG							m_lCellScore;						//单元分数
	WORD							m_wCurrentTime;						//当前倍数
	WORD							m_wBackCardTime;					//底牌倍数
	BYTE							m_cbBombCount;						//炸弹个数
	BYTE							m_cbBackCardType;					//底牌类型	
	BYTE							m_cbCallBankerInfo[GAME_PLAYER];	//叫地主信息
	BYTE							m_cbRodBankerInfo[GAME_PLAYER];		//抢地主信息
	//BYTE							m_cbAddDoubleInfo[GAME_PLAYER];		//加倍信息	
	CRect							m_rcTime;							//倍数位置
...
public:
	...
	//功能按钮
public:
	CSkinButton					m_btCancelTrustee;					//取消托管按钮
	CSkinButton					m_btTrustee;						//托管按钮
	CSkinButton					m_btLastTurn;						//上轮按钮
	CSkinButton					m_btSortCard;						//排序按钮

	//叫分按钮
public:
	//CSkinButton					m_btCallScore1;						//叫分按钮
	//CSkinButton					m_btCallScore2;						//叫分按钮
	//CSkinButton					m_btCallScore3;						//叫分按钮
	//CSkinButton					m_btCallScoreNone;					//叫分按钮
	//地主按钮
public:
	CSkinButton					m_btCallBanker;						//叫地主按钮
	CSkinButton					m_btNoCallBanker;					//不叫按钮
	CSkinButton					m_btRodBanker;						//抢地主按钮
	CSkinButton					m_btNoRodBanker;					//不抢按钮



	//提示按钮
public:
	CSkinButton					m_btCardPrompt1;					//提示按钮
	CSkinButton					m_btCardPrompt2;					//提示按钮
	CSkinButton					m_btCardPrompt3;					//提示按钮
	CSkinButton					m_btCardPrompt4;					//提示按钮
	CSkinButton					m_btCardPrompt5;					//提示按钮
	CSkinButton					m_btCardPrompt6;					//提示按钮
...
protected:
	...
	CD3DTexture						m_TextureGameOver;					//结算资源
...
protected:
	...
	CD3DTexture						m_TextureCallBanker;				//用户叫地主
	CD3DTexture						m_TextureRodBanker;					//用户抢地主
...
public:
	...
	CDoubleControl					m_DoubleControl;					//倍数控件
