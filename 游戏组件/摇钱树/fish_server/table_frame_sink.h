...
class TableFrameSink : public ITableFrameSink, public ITableUserAction {
 public:
  ...
  virtual bool OnActionUserOffLine(WORD chair_id, IServerUserItem * server_user_item);
  //用户重入
 ...
 private:
  ...
  //上分
  bool OnSubExchangeFishScore(IServerUserItem* server_user_item, int increase);
  //射击
 ...
 private:
  ...
  int CreateFishKind_Time[CREATE_KIND_COUNT];					//生成各种鱼的时间
  int CreateFishKind_Min_Count[CREATE_KIND_COUNT];				//生成鱼的数目的最小值
  int CreateFishKind_Max_Count[CREATE_KIND_COUNT];				//生成鱼的数目的最大值

  int bullet_speed_[BULLET_KIND_COUNT];	//子弹速度
  int net_radius_[BULLET_KIND_COUNT];	//渔网半径
  int IonTimer_;						//离子炮时间
  ...
  int BulletIonTimeCount[GAME_PLAYER];
