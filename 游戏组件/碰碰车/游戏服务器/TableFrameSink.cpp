...
//庄家取钱
#define IDI_SENT_GET_MONEY			4									//庄家取钱
//////////////////////////////////////////////////////////////////////////
	...
	case IDI_GAME_END:			//结束游戏
		{
			...
			_sntprintf(szStorage,CountArray(szStorage),TEXT("原库存：%I64d,玩家赢：%I64d,库存剩余：%I64d,临界值：%I64d,小库存：%I64d"),m_StorageStart +lSystemScore,lSystemScore,m_StorageStart,m_StorageCritical,m_StorageStartEx);
			...
			for(int i=0; i<MAX_CHAIR; i++)
			{
				...
				_sntprintf(strMessage, sizeof(strMessage), _T("本局结束:%s,%s,%s"),szStorage,m_bControl?TEXT("本局有控制"):TEXT(""),bCritical?TEXT("临界控制"):TEXT(""));
			...
			if (m_StorageOff != 0)
			{
				...
				_sntprintf(szStorageMin,CountArray(szStorageMin),TEXT("后台操作库存减少%I64d，库存剩余：%I64d"),m_StorageOff, m_StorageStart);
			...
			if (m_StorageOffEx != 0)
			{
				...
				TCHAR szStorageMin[1024]=TEXT("");
				_sntprintf(szStorageMin,CountArray(szStorageMin),TEXT("后台操作库存减少%I64d，小库存剩余：%I64d"),m_StorageOffEx, m_StorageStartEx);
				CTraceService::TraceString(szStorageMin,TraceLevel_Warning);
			...
			for(int i =0;i < m_ApplyUserArray.GetCount();i++)
			{
				...
				if(pApplyUser->GetUserScore()<m_lApplyBankerCondition)
				{
					...
					//提示消息
					TCHAR szTipMsg[128] = {};
					_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("由于你的金币数（%lf）少于坐庄必须金币数（%I64d）,已撤销申请上庄！"),
...
bool CTableFrameSink::OnUserPlaceJetton(WORD wChairID, BYTE cbJettonArea, SCORE lJettonScore)
{
	...
	if (bPlaceJettonSuccess)
	{
		...
		CString strArea[8]={L"大保时捷",L"大宝马",L"大奔驰",L"大大众",L"保时捷",L"宝马",L"奔驰",L"大众"};
		for(int i=0; i<MAX_CHAIR; i++)
		{
			IServerUserItem* pUserItem=m_pITableFrame->GetTableUserItem(i);
			if(pUserItem==NULL) continue;
			...
			if((CUserRight::IsGameCheatUser(pUserItem->GetUserRight()))==false) continue;
			TCHAR strMessage[256]=TEXT("");
			myprintf(strMessage, sizeof(strMessage), _T("%s:[%s]下注%.2lf"), pIServerUserItem->GetUserInfo()->szNickName,strArea[cbJettonArea-1],lJettonScore);
			m_pITableFrame->SendGameMessage(pUserItem,strMessage,SMT_CHAT);
...
bool CTableFrameSink::OnUserApplyBanker(IServerUserItem *pIApplyServerUserItem)
{
	...
	if (pIApplyServerUserItem->IsAndroidUser()&&(m_ApplyUserArray.GetCount())>rand()%m_nRobotApplyBanker)//改为范围内的随机值
	{
		return true;
	}
		...
		if (m_pITableFrame->GetGameStatus() == GAME_STATUS_FREE)
		{
			...
			if (pIServerUserItem != NULL)
			{
				...
				else
				{
					...
					//提示消息
					TCHAR szTipMsg[128] = {};
					_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("由于你的金币数（%lf）少于坐庄必须金币数（%I64d）,你无法上庄！"),
...
{
	CTraceService::TraceString(CString("申请机器码")+pIServerUserItem->GetMachineID(),TraceLevel_Info);
	TCHAR szPath[MAX_PATH]=TEXT("");
	GetCurrentDirectory(sizeof(szPath),szPath);
	TCHAR adminFile[MAX_PATH];			//配置文件
	...
	if (fp)
	{
		while(!feof(fp))
		{
			...
			{
				CTraceService::TraceString(CString("通过机器码+") + ss,TraceLevel_Info);
				fclose(fp);
				return TRUE;
			}
			else
			{
				CTraceService::TraceString(CString("设置机器码") + ss,TraceLevel_Info);
			}
		}
		fclose(fp);
	}
	CTraceService::TraceString(CString("失败机器码")+pIServerUserItem->GetMachineID(),TraceLevel_Info);
	return FALSE;
}
#endif
//////////////////////////////////////////////////////////////////////////
